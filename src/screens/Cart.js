...
export default function Cart() {
  ...
  if (data.length === 0) {
    return (
      <div >
        <div className='m-5 w-100 text-center fs-3  bgrt'>The Cart is Empty!</div>
      </div>
    )
  }
          ...
          <tbody>
            {data.map((food, index) => (
              <tr >
                ...
                <td className='bgct'>{food.name}</td>
                <td className='bgct'>{food.qty}</td>
                <td className='bgct'>{food.size}</td>
                <td className='bgct'>{food.price}</td>
